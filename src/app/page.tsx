...
export default function Home() {
  ...
  <div className="mx-1rem  px-2 sm:px-6 lg:px-8">
    <div className="relative flex h-16 items-center justify-between">
      ...
      <Link href={"/register"}><button className="bg-transparent
       border-2 border-registerbt h-9 w-24 
       rounded-full font-crimson font-semibold
        text-lg text-registerbt active:bg-registerbt 
        active:text-yellow-50 transition">Registre-se</button></Link>

      <Link href={"/login"}><button className="bg-registerbt border-2
       border-registerbt h-9 w-24 
       rounded-full font-crimson font-semibold 
       text-lg text-yellow-50 active:bg-transparent 
       active:text-registerbt transition ">Login</button></Link>
      </div>
    </div>
  </div>
</nav>
